...
@pytest.fixture
def other_user(db):
    user = User.objects.create_user(email="user2@user2.com",password="@#1234567",is_verified=True)
    return user
...
class TestTaskApi:
    ...
    def test_partial_update_task_response_200_status(self,api_client,common_user,task):
        url = reverse('todo:api-v1:task-detail', kwargs={'pk':task.id})
        data = {
            "title" : "partial-update-task",           
        }
